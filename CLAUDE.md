...
- Language: SQL
- File Extensions: .sql
- Metadata: header comments using `-- @title:`, `-- @description:`, `-- @tags:`
- Use Case: Sample queries for TagoIO SQL API routes
...
- pnpm (see `packageManager` in `package.json`; npm and yarn are not supported)
- OXC: oxlint and oxfmt (installed as devDependencies)
...
# Build JSON data and static site (runs prepare-data.ts then astro build)
pnpm build

# Preview built site locally
pnpm preview

# Format all code (JS/TS with oxfmt, Python with Ruff)
pnpm fmt
# JS/TS only
pnpm fmt:js
# Python only
...
# Lint all code (JS/TS with oxlint, Python with Ruff)
pnpm lint
# JS/TS only
pnpm lint:js
# Python only
...
# Check (oxlint deny-warnings, oxfmt --check, ruff)
pnpm check
```
...
scripts/
└── prepare-data.ts       # Generates src/data/snippets.json

src/
├── data/snippets.json    # Generated at build/start
└── pages/
   ├── analysis/[runtime].json.ts
   ├── analysis/[runtime]/[filename].ts
   ├── payload-parser/[runtime].json.ts
   ├── payload-parser/[runtime]/[filename].ts
   ├── tagosql/[runtime].json.ts
   └── tagosql/[runtime]/[filename].ts
...
Category-based endpoints (preferred):
...
- Automatic: GitHub Actions builds with pnpm and deploys the Astro site to GitHub Pages on push to main
- Manual: Run `pnpm build` locally and deploy the `dist/` output
...
```bash
# OXC tools ship as package devDependencies (oxlint, oxfmt)
pnpm install
